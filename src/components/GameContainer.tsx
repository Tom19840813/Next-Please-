...
const GameContainer: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="absolute top-4 left-4 z-50 bg-white bg-opacity-80 px-3 py-1 rounded-full text-game-purple font-bold">
        Score: {score}
      </div>
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-50">
        {GAMES.map((game, index) => (
          <div
            key={game}
            className={`h-2 w-2 rounded-full ${currentGame === game ? 'bg-game-purple' : 'bg-gray-300'}`}
          />
        ))}
      </div>
      ...
      <button 
        ...
        className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-60 hover:bg-opacity-80 p-2 rounded-full z-40"
      ...
      <button 
        ...
        className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-60 hover:bg-opacity-80 p-2 rounded-full z-40"
